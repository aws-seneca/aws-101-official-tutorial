...
const QUESTIONS: { q: string; options: string[]; answer: number; why: string }[] = [
  ...
  { q: "Your sign-up form says it can't reach the database. What fixes it?", options: ["Attach an IAM role to the server", "Make the database publicly accessible", "Allow port 5432 on the database's security group, with the server's security group as source", "Reboot the database"], answer: 2, why: "Reaching a port is network access. The database's security group has to let the server in." },
